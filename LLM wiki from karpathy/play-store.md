...
- Latest built bundle: **versionCode 101 / versionName 61** (2026-08-02) — adds **RECORD_AUDIO + MODIFY_AUDIO_SETTINGS** for the mic/dictation feature; keeps native alarms (verified in the .apk: RECORD_AUDIO, POST_NOTIFICATIONS, USE_FULL_SCREEN_INTENT, AlarmActivity, AlarmReceiver). File: `store/assets/DailyPulse.aab`. **⏳ NOT yet uploaded to Play — user must upload this to the closed-testing track** (testers then get an in-app update; only native changes need this — web pushes are automatic). Previous uploaded build was 100/60.
